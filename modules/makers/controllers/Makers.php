...
class Makers extends Trongate {
    ...
    public function create(): void {
        ...
        if ($update_id>0) {
            $data['headline'] = 'Update Maker Record';
            $data['cancel_url'] = BASE_URL.'makers/show/'.$update_id;
        } else {
            $data['headline'] = 'Create New Maker Record';
            $data['cancel_url'] = BASE_URL.'makers/manage';
        }

        $data['form_location'] = BASE_URL.'makers/submit/'.$update_id;
    ...
    public function manage(): void {
        ...
        if (segment(4) !== '') {
            ...
            $params['maker_name'] = '%'.$searchphrase.'%';
            $params['support_number'] = '%'.$searchphrase.'%';
            $sql = 'select * from makers
            WHERE maker_name LIKE :maker_name
            OR support_number LIKE :support_number
            ORDER BY id';
            $all_rows = $this->model->query_bind($sql, $params, 'object');
        } else {
            $data['headline'] = 'Manage Makers';
        ...
        $pagination_data['pagination_root'] = 'makers/manage';
        $pagination_data['record_name_plural'] = 'makers';
    ...
    public function show(): void {
        ...
        if ($data == false) {
            ...
            $data['headline'] = 'Maker Information';
            $data['view_file'] = 'show';
            $this->template('admin', $data);
        }
    }
    ...
    public function submit(): void {
        ...
        if ($submit == 'Submit') {
            ...
            $this->validation->set_rules('maker_name', 'maker name', 'required|min_length[2]|max_length[255]');
            $this->validation->set_rules('support_number', 'support number', 'min_length[2]|max_length[255]');
            $this->validation->set_rules('notes', 'notes', 'min_length[2]');
            ...
            if ($result == true) {
                ...
                if ($update_id>0) {
                    ...
                    $this->model->update($update_id, $data, 'makers');
                    $flash_msg = 'The record was successfully updated';
                } else {
                    //insert the new record
                    $update_id = $this->model->insert($data, 'makers');
                    $flash_msg = 'The record was successfully created';
                }

                set_flashdata($flash_msg);
                redirect('makers/show/'.$update_id);
    ...
    public function submit_delete(): void {
        ...
        if (($submit == 'Yes - Delete Now') && ($params['update_id']>0)) {
            ...
            $params['module'] = 'makers';
            $this->model->query_bind($sql, $params);

            //delete the record
            $this->model->delete($params['update_id'], 'makers');
    ...
    private function get_data_from_db(int $update_id): ?array {
        $record_obj = $this->model->get_where($update_id, 'makers');
    ...
    private function get_data_from_post(): array {
        $data['maker_name'] = post('maker_name', true);
        $data['support_number'] = post('support_number', true);
        $data['notes'] = post('notes', true);        
        return $data;
    }
